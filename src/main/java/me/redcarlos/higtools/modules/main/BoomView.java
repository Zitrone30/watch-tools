...
public class BoomView extends Module {
    ...
    public final Setting<BoomView.Mode> mode = sgGeneral.add(new EnumSetting.Builder<BoomView.Mode>()
        .name("mode")
        .description("Dodging mode.")
        .defaultValue(BoomView.Mode.TP)
        .build()
    );
    ...
    private final Setting<Boolean> sendonground = sgGeneral.add(new BoolSetting.Builder()
        .name("OnGround")
        .description("Sends the server onground packets")
        .defaultValue(false)
        .build()
    );

    private final Setting<Boolean> horizcol = sgGeneral.add(new BoolSetting.Builder()
        .name("Horizontal Collisions")
        .description("Enable/Disable Horizontal collisions, usually used for testing.")
        .defaultValue(false)
        .build()
    );
    ...
    public void clip() {
        // Server-side jump happens instantly
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(
            mc.player.getX(), mc.player.getY() + 1.0, mc.player.getZ(), sendonground.get(), horizcol.get()
        ));

        // Delay the client-side jump by 4 seconds
        new Thread(() -> {
            try {
            ...
            // Client-side jump (only happens once)
