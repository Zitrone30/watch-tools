...
public class AutoLogPlus extends Module {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof PlayerEntity && entity.getUuid() != mc.player.getUuid()) {
                if (onlyTrusted.get() && entity != mc.player && !Friends.get().isFriend((PlayerEntity) entity)) {
                    disconnect(Text.of("[AutoLogout] A non trusted player [" + entity.getName().getString() + "] has entered your render distance."));
                }
            }
    ...
    private void disconnectOnHighPing() {
        ...
        if (ping >= pingValue.get()) disconnect(Text.of("[AutoLogout] High ping [" + ping + "]"));
    }
